...
void process_setup(pid_t pid, const char *program_name)
{
    ...
    for (auto seg = pgm.begin(); seg != pgm.end(); ++seg)
    {
             ...
             a += PAGESIZE)
        {
            ...
            void *p = kalloc(a / PAGESIZE);
            memory_map(ptable[pid].pagetable, a, (uintptr_t)p, PTE_PWU);
        }
    }
    ...
    void *p = kalloc(stack_addr / PAGESIZE);
    memory_map(ptable[pid].pagetable, stack_addr, (uintptr_t)p, PTE_PWU);
...
int syscall_page_alloc(uintptr_t addr)
{
    void *p = kalloc(addr / PAGESIZE);
    if (p == nullptr)
    {
        return -1;
    }
    memory_map(current->pagetable, addr, (uintptr_t)p, PTE_PWU);
